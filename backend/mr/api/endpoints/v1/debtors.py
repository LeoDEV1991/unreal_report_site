...
def get_debtors_summary():
    ...
    query = g.s.query(DD.FY, DD.Month, FFM.DebtorDays,DD.FirstDayOfMonth)\
        .join(DD)\
        .outerjoin(DO, FFM.DimOrganisationKey == DO.DimOrganisationKey)

    if 'years' in filter_data:
        query = query.filter(DD.FY.in_(filter_data['years']))

    summary = query.group_by(DD.FY,DD.Month,FFM.DebtorDays,DD.FirstDayOfMonth).all()
...
def get_debtors_detail():
    ...
    query = g.s\
        ...
        .outerjoin(DED, FARI.DimEmployeeDirectorKey == DED.DimEmployeeKey )\
        .outerjoin(DEPM, FARI.DimEmployeeProjectManagerKey == DEPM.DimEmployeeKey )\
        .outerjoin(DESU, FARI.DimEmployeeSupervisorKey == DESU.DimEmployeeKey )\
        .join(DD)\
