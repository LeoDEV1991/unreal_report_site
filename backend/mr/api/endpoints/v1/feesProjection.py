...
@api.get('/feesprojection/summary')
@api.doc("""
Get feesprojection summary data.
""")
# @auth.required
def get_feesprojection_summary():
    ...
    query = g.s\
        .query(
            ...
            func.sum(GLIS.Actual).label("ActualFess"),
            func.sum(GLIS.Forecast).label("ForecastFees"),
            func.sum(GLIS.Budget).label("BudgetedFees")
        )\
        .join(DO, GLIS.DimOrganisationKey == DO.DimOrganisationKey)\
        .join(DD, GLIS.DimDateKey == DD.DimDateKey)
    ...
    for row in summary:
        ...
        item['BudgetFees'] = row[6]
        resp.append(item)

    return resp


@api.get('/feesprojection/detail')
@api.doc("""
Get feesprojection detail data.
""")
# @auth.required
def get_feesprojection_detail():
    ...
    query = g.s\
        ...
        .outerjoin(DEPM, FPF._ProjectManager)\
        .outerjoin(DESU, FPF._Supervisor)\
        .join(DPT).join(DPST)
