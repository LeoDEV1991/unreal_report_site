...
const CashFlow = () => {
  ...
  const getTop= (selectedYears) => {
    ...
    return _api.get('v1.0/cashflow/topdata' + queryParams);
  };

  const getMiddle= (dimDate) => {
    let queryParams = '';

    let _selectedDimDate = dimDate.slice();
    const filter = {DimDateKey: _selectedDimDate};

    queryParams += ('?filter=' + JSON.stringify(filter));

    return _api.get('v1.0/cashflow/middledata' + queryParams);
  };
  ...
  const getBottom= (selectedYears) => {
    ...
    return _api.get('v1.0/cashflow/bottomdata' + queryParams);
  };
  return {
    getTop,
    getMiddle,
