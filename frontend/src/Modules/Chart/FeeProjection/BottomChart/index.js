...
class BottomChart extends Component {
    ...
    detailData.forEach(d => {
      if (
        ...
        if (!dictData[y(d)]) {
          ...
          dictData[y(d)]['ProjectType'] = d['ProjectType'];
  ...
  render() {
    ...
    let { data } = this.state;
    data = data.sort((a,b) => ( (-(a.ActualFees /*+ a.ForecastFees*/) + (b.ActualFees /*+ b.ForecastFees*/) ) ));
                    ...
                    >
                      {tooltipData.isBar ?
                        <div>
                          ...
                          <div className="ft-12">Project Type: {(xType(tooltipData))}</div>
