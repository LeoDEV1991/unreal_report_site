...
class TopChart extends Component {
    ...
    if(topData && topData.length > 0 && selectedYears.length > 0)
    {
      topData.forEach(item => {
        let month = getMonth(item.Date);
          ...
          _minLine = Math.min(_minLine , item.CashBalance);
      });
  ...
  _handleLineSelect = (event, item , xScale, yScale) => {
    ...
    let { selectedTopItems, dimDate , cashflowMiddleRequest , cashflowBottomRequest} = this.props;
    ...
    cashflowMiddleRequest([item.year], dimDate);
    let years = [];
    years = (selectedTopItems.map(d => d.year));
    if(years.length > 0)
      cashflowBottomRequest(years);
  };
  ...
  render() {
    ...
    let { data , safetyLine , minLine, maxLine, selectedLines} = this.state;
        ...
        <div className={classes.frame}>
            <Grid container  style={{borderLeft: 'solid 1px #a9a9a9'}} >
              ...
              {data.map((item , key) => {
                ...
                const offsetX = 3.5;
                let futureWidth , futureX;
                let pastWidth = 0;
                if(this.props.defaultYear < item.year && selectedYears.indexOf(this.props.defaultYear) === -1 )
                {
                  futureWidth = localWidth;
                  futureX = 0;
                }
                    ...
                    <div  key = {key}>
                      <Grid container style={{borderRight: 'solid 1px #a9a9a9'}}>
                        <svg width={localWidth} height={localHeight}>
                          ...
                          {
                            (item.year >= this.props.defaultYear || selectedYears.indexOf(this.props.defaultYear) > -1)?
                              <Group>
                                <Bar
                                  width={futureWidth}
                                  height={localHeight}
                                  x={futureX}
                                  y={0}
                                  fill={item.year===this.props.defaultYear?'#f0f0f0':'#ffffff'}
                                  stroke={"black"}
                                  strokeWidth={0}
                                />
                              </Group>
                          ...
                          <rect x={0} y={0} width={localWidth } height={localHeight} fill={'transparent'}
                                onMouseMove={event => {
                                  if (tooltipTimeout) clearTimeout(tooltipTimeout);
                                  this._showTooltip(
                                    event,
...
TopChart.propTypes = {
  ...
  cashflowBottomRequest: PropTypes.func.isRequired,
