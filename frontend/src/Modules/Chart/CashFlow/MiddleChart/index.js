...
class MiddleChart extends Component {
  ...
  _prepareData = () => {
    ...
    if(middleData)
    {
      ...
      middleData.forEach(item => {
          ...
          if(/*item.CashChange !== 0 &&*/ item.ForecastCashChange != 0 ) {
            ...
            data[item.CashFlowCategory]["ForecastCashChange"] += item.ForecastCashChange;
          }
        });
    }
    ...
    _data.sort( (a,b) => ( b.ForecastCashChange - a.ForecastCashChange));
  ...
  render() {
    ...
    data.map( (item , key) => {
      minData += item.ForecastCashChange;
      maxData = Math.max(item.ForecastCashChange , maxData );
    });
    ...
    return (
      ...
      <div className={classes.root}  id = "middleRoot">
        <div>
          <Typography variant="h6" className="subtitle mt-10">Selected Cash Flow</Typography>
        </div>
                        ...
                        <Grid item md={10} sm={10} xs={10} style={{borderTop:'solid 1px #a9a9a9',borderRight:'solid 1px #a9a9a9'}}>
                          {
                            ...
                            data.map( (item , j) => {
                              ...
                              if(item.ForecastCashChange < 0)
                              {
                                _maxData += item.ForecastCashChange;
                                barX = xScale(_maxData);
                                barWidth = xScale(_minData) - barX;
                                _minData += item.ForecastCashChange;
                              }
                              else
                              {
                                _maxData += item.ForecastCashChange;
            ...
            <Tooltip
              ...
              {tooltipData.isBar ?
                <div style = {{width:'200px'}}>
                  ...
                  <div className="flex">
                    <strong> {thousandFormat2(tooltipData.ForecastCashChange)}</strong>
                  </div>
                </div>
                :
                ""
